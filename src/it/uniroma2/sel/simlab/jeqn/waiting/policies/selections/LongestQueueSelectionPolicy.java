...
/** Implements a selection policy that picks up the longest queue
 *
 * @author Daniele Gianni
 */
public class LongestQueueSelectionPolicy extends ImplicitButNotExplicitInputOnlyDependentPolicy<UserMultiQueue, UserQueue>  {
    
    /**
     * Creates a new instance of ShortestQueueDispatchingPolicy
     * @param userMultiQueue The multi queue upon which the decision is made
     */
    public LongestQueueSelectionPolicy(final UserMultiQueue userMultiQueue) {
        super(userMultiQueue);
    }

    public UserQueue getDecision() {
        
        UserQueue longestQueue = implicitInput.getUserQueue(0);
        
        for (UserQueue uq : implicitInput.getUserQueues()) {
            if (uq.getEnqueuedUsers() > longestQueue.getEnqueuedUsers()) {
                longestQueue = uq;
            }
        }
